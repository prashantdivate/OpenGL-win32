...
void PMS_display(void)
{
	...
	float numberOfCircles = 8.0f;
	// turn on wireframe mode
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);

	for (float j = 1.0f; j <= numberOfCircles; j += 1.0f)
	{
		circle(radius);
		radius = radius - 0.1f;

		if (j == numberOfCircles)
			radius = 1.0f;
	}
...
{
	glBegin(GL_POINTS);
	for (float i = 0.00001f; i <= 2 * PI; i += 0.00001)
	{
		glVertex3f(radius * cos(i), radius * sin(i), 0.0f);
	}
	glEnd();
}
void PMS_uninitialize(void)
