	...
	int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Creation of deathy hallows"), // Title of window
...
void PMS_display(void)
{
	static float Tx_pos = 3.0f;
	static float Ly_pos = 3.0f;
	static float Cx_pos = 3.0f;
	...
	glTranslatef(Tx_pos, -Tx_pos, -6.0f);
	glRotatef(angle, 0.0f, 1.0f, 0.0f);
	triangle();
	
	glLoadIdentity();
	glTranslatef(0, Ly_pos, -6.0f);
	glBegin(GL_LINES);
	...
	glLoadIdentity();
	glTranslatef(-Cx_pos, -Cx_pos - 0.38f, -6.0f);
	glRotatef(angle, 0.0f, 1.0f, 0.0f);
	circle();
...
void circle(void)
{
	float radius;
	float side = 2.0f;
	//radius = side / (2.0f * 1.73205080757f);
	radius = 0.62;
	glBegin(GL_POINTS);
	for (float i = 0.0001f; i <= 2 * PI; i += 0.00001f)
	{
