	...
	int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Multiple viewports"), // Title of window
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	void PMS_display(void);
	static unsigned int curWidth, curHight;
	switch (iMsg)
	{
	case WM_SETFOCUS:
		gbActive = true;
	...
	case WM_ERASEBKGND:
		return(0);
	case WM_SIZE:
		PMS_resize(LOWORD(lParam), HIWORD(lParam));  //width and height
		curWidth = LOWORD(lParam);
		curHight = HIWORD(lParam);
		break;
	case WM_CLOSE:
		DestroyWindow(hwnd);
		break;
	case WM_CHAR:
		msgView = wParam;
		if (msgView >= '0' && msgView <= '9')
		{
			PMS_resize(curWidth, curHight);
		}
		break;
	case WM_KEYDOWN:
		switch (wParam)
...
void PMS_resize(int width, int height)
{
	...
	{
	case '0':
		glViewport(0, 0, (GLsizei)width, (GLsizei)height);  //normal viewport
		break;
	case '1':
		glViewport(0, 0, (GLsizei)width / 2, (GLsizei)height / 2);  // left bottom
		break;
	case '2':
		glViewport((GLsizei)width / 2, 0, (GLsizei)width / 2, (GLsizei)height / 2);  // right bottom
		break;
	case '3':
		glViewport((GLsizei)width / 2, (GLsizei)height / 2, (GLsizei)width / 2, (GLsizei)height / 2);  // right top
		break;
	case '4':
		glViewport(0, (GLsizei)height / 2, (GLsizei)width / 2, (GLsizei)height / 2);  // left top
		break;
	case '5':
		glViewport(0, 0, (GLsizei)width / 2, (GLsizei)height);  // left half
		break;
	case '6':
		glViewport((GLsizei)width / 2, 0, (GLsizei)width / 2, (GLsizei)height);  // right half
		break;
	case '7':
		glViewport(0, (GLsizei)height / 2, (GLsizei)width, (GLsizei)height / 2);  // upper half
		break;
	case '8':
		glViewport(0, 0, (GLsizei)width, (GLsizei)height / 2);  // bottom half
		break;
	case '9':
		glViewport((GLsizei)width / 4, (GLsizei)height / 4, (GLsizei)width / 2, (GLsizei)height / 2);  // viewport centre
		break;
	}
